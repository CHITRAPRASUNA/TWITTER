...
const getFollowPeopleIdsOfUser = async (username) => {
  const getTheFollowPeopleQuery = `
    SELECT 
    follower_user_id FROM follower
    INNER JOIN user ON user.user_id = follower.follower_user_id
    WHERE user.username = '${username}';`;

  const followingPeople = await db.all(getTheFollowPeopleQuery);
...
const authentication = (request, response, next) => {
  ...
  if (jwtToken) {
    jwt.verify(jwtToken, "SELECT_KEY", (error, payload) => {
      if (error) {
        response.status(401);
...
app.post("/login/", async (request, response) => {
  ...
  if (userDbDetails !== undefined) {
    ...
    if (isPasswordCorrect) {
      ...
      const jetToken = jwt.sign(payload, "SECRET_KEY");
...
app.get(
  ...
  async (request, response) => {
    ...
    const likedQuery = await db.all(getLikesQuery);
...
app.post("/user/tweets/", authentication, async (request, response) => {
  ...
  const dateTime = new Date().JSON.substring(0, 19).replace("I", " ");
...
app.delete("/tweets/:tweetId/", authentication, async (request, response) => {
  const { tweetId } = request.params;
  const { userId } = request;
  const getTheTweetQuery = `SELECT * FROM tweet WHERE 
